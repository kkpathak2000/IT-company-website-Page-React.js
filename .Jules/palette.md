...
## 2025-06-08 - Keyboard Navigation and Skip Links
**Learning:** For single-page applications with fixed navigation bars, a "Skip to main content" link is essential for keyboard accessibility. It must be the first focusable element in the DOM and requires a matching landmark with `tabIndex="-1"` to ensure focus is correctly moved in all browsers.
**Action:** Always include a skip link as the first child of the main container and wrap primary content in a `<main id="main-content" tabIndex="-1">` landmark.
